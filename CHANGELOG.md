...
### Added
- **Brain MCP integration documentation** (post-release doc-sync): new `docs/guidelines/BRAIN_MCP_INTEGRATION_GUIDELINES.md` covering the optional external Brain MCP server connection protocol, retrieval-path differences (`brain_ask_oracle` cross-project vs `brain_retrieve_knowledge` project-scoped), auto-mode classifier guardrails (single-call allowed / batch-framed denied / settings.json permission self-grant blocked), and recovery procedure when the MCP transport drops. Adds corresponding sections to `docs/KNOWLEDGE_MANAGEMENT.md`, `docs/KNOWN_ISSUES.md`, `docs/APPROACH_AND_METHOD.md`, `README.md`, and `CLAUDE.md`. Transferred 15 durable rules from the v8.4.5 release session to two Brain projects (`LLM-Autonomous-Agent-Plugin-for-Claude` and `AutonomousAgent`) as a one-time backfill.
- **Brain MCP oracle ranking behavior** (known issue): `brain_ask_oracle` under-ranks newly-taught project-scoped knowledge items (cold-start penalty + scope filter), while `brain_retrieve_knowledge` with explicit `projectId` reliably surfaces them. Documented in `docs/KNOWN_ISSUES.md` with the workaround.
