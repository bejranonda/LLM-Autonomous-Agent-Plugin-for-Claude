...
- **Brain MCP oracle under-ranks new project-scoped knowledge (NOT part of this plugin)**: When using the external Brain MCP server (optional, separate service), `brain_ask_oracle(query)` may return "no knowledge" for items you taught minutes ago, while `brain_retrieve_knowledge(query, projectId=<id>)` surfaces them with similarity 0.50-0.88. Root cause: oracle ranking applies a cold-start penalty to items with `usageCount: 0` and favors user-scoped over project-scoped items. Workaround: query with explicit `projectId`. Ranking improves naturally as items accrue `usageCount`. See `docs/guidelines/BRAIN_MCP_INTEGRATION_GUIDELINES.md`.
- **Brain MCP transport drops without auto-reconnect (NOT part of this plugin)**: The Brain MCP client SDK does not auto-reconnect on server restart, 30-min orphan sweep, or network blip. Symptoms: tools return "Server not initialized" or disappear from the function list. Recovery: `/mcp` -> reconnect, or fully restart Claude Code (not just `/compact`). Retrying a failing call in a loop will not recover the session.
- **Auto-mode classifier blocks Brain MCP settings.json permission self-grants (NOT a bug)**: When `permissions.defaultMode: "auto"`, the classifier prevents agents from editing `~/.claude/settings.json` to add `mcp__brain__*` entries to `permissions.allow`. This is a deliberate permission-self-grant guardrail, not a classifier bug. To add Brain tools to your allow list, edit the file manually. See `docs/guidelines/BRAIN_MCP_INTEGRATION_GUIDELINES.md`.
